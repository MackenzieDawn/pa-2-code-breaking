...
# Creates letters and punctuation's
...
```{r}
#| label: One-Vector
# Adding all vectors into one
my_symbols <- tibble(symbols= c(lower_case, upper_case, punctuation)) |> mutate(num = row_number())
...
# adding, multiplying, exponentiating, and squaring every number
# there are several ways to do this, the way below doesn't overwrite any column so you "don't lose work"

code_break <- top_secret |> 
  as_tibble() |>
  mutate(step6 = top_secret + 14, step7 = step6 * 18 - 257, step8 = exp(step7), step9 = step8 ^2)
...
# There should be 352 numbers below 17, the code is to verify

code_break |>
  summarise(total = sum(step9 < 17))
...
```{r}
#| label: Matrix
# Turning vector into a matrix with 5 columns
code_matrix <- code_break |> pull(step9) |> matrix(ncol = 5) |> as_tibble(column_name = "V") |> mutate(id = row_number())
